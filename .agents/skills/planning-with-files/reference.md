...
|------|---------|--------------|--------------|
| `task_plan.md` | Goal, phases, current phase, open questions (the plan — kept short) | Task start | When a phase's overall status changes |
| `findings.md` | Discoveries, constraints, decisions + rationale, wrong turns | After ANY discovery | After viewing images/PDFs, after any decision |
| `progress.md` | Session log: actions, files touched, tests, commits | At breakpoints | Throughout session |
| Code files | Implementation | Before execution | After errors |

> Mapping to the principles: `task_plan.md` is the **recitation** surface (Principle 4) — it must
> stay short to keep the goal in the attention window, so it only describes each phase, never the
> execution. `progress.md` is the append-only action log. `findings.md` is the **external memory**
> (Principle 3) and the home for **kept-in wrong turns** (Principle 5) — but Principle 3 also says
> *compression must be restorable*: store the key points and a **pointer** to any full source on
> disk, never a transcription of it. Big ≠ good; restorable = good.
