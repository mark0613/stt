...
def has_planning_files(project_path: str) -> bool:
    planning_root = Path(project_path) / ".planning"
    if not planning_root.is_dir():
        return False
    return any(path.is_file() for path in planning_root.glob("*/task_plan.md"))
...
def opencode_catchup(project_path: str) -> None:
    ...
    print("2. Read: current .planning/<plan>/task_plan.md, progress.md, findings.md")
